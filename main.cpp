...
bool isValid(vector<string> command){
	...
	if(commandType == -1)
		 return false;
    if(commandType == 1 or commandType == 3 or commandType == 4 or commandType == 6){
...
{ 
  ...
  while (getline(cin, command)){
    ...
    if(!isValid(commandWithParams)){ // invalid command. please note commands are case sensitive
